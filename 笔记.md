...
9. vue axios设置注意：如果创建axios这里用的是/api，也就是说没有写具体的地址，那么就会在前端起一个服务，把请求都代理到这个地址，再转发到实际的配置的后端地址，那么在vue.config.js那里就可以正常的配置代理,请求的地址就是http://localhost:3000/api/list，如果axios实例这里写的是http://localhost:3000, 这个是我node起的一个服务地址，那么vue.config.js 就不需要配置代理，实际的请求地址就是http://localhost:3000/list,注意这里的一个区别
